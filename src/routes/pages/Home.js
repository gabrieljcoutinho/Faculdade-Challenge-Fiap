...
const WEATHER_ICONS = {
  ...
  Limpo: '☀️',
  Tempestade: '⛈️',
  'Noite Limpa': '🌙' // Adicionado novo ícone para a lua
};
...
const Home = () => {
  ...
  const getWeatherIcon = useCallback(c => {
    ...
    // Verifica se a condição é de céu limpo e se é noite
    if ((normalizedCondition.includes('céu limpo') || normalizedCondition.includes('limpo') || normalizedCondition.includes('sol')) && isNight()) {
      return (
        <span className="weather-icon moon-animation">
          {WEATHER_ICONS['Noite Limpa']}
        </span>
      );
    }

    // Lógica para as outras condições
    if (normalizedCondition.includes('sol') || normalizedCondition.includes('limpo')) {
      return (
        <span className="weather-icon sun-animation">
          {WEATHER_ICONS['Ensolarado']}
        </span>
      );
    } else if (normalizedCondition.includes('nublado')) {
      return (
        <span className="weather-icon cloud-animation">
          {WEATHER_ICONS['Nublado']}
        </span>
      );
    } else if (normalizedCondition.includes('chuva')) {
      return (
        <span className="weather-icon rain-cloud-animation">
          {WEATHER_ICONS['Chuvoso']}
        </span>
      );
    } else if (normalizedCondition.includes('tempestade')) {
      return (
        <span className="weather-icon storm-animation">
          {WEATHER_ICONS['Tempestade']}
        </span>
      );
    } else {
      return (
