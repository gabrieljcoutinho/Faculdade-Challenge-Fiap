...
ChartJS.register(
  ...
  Legend,
  Tooltip,
  Filler // importante para o preenchimento do gráfico
);
...
const Home = () => {
  ...
  // Aqui a referência do gráfico - usada para salvar imagem
  ...
  const getChartOptions = useCallback(
    (type) => {
      const baseOpts = {
        ...
        plugins: {
          ...
          title: {
            ...
            text:
              type === 'pie'
                ? 'Distribuição da Produção de Energia Solar por Hora'
                : `Produção de Energia Solar por Hora (${type === 'line' ? 'Linha' : 'Barras'})`,
      ...
      if (type === 'pie') {
        ...
        baseOpts.plugins.tooltip = {
          ...baseOpts.plugins.tooltip,
          callbacks: { label: (ctx) => `${ctx.dataset.label || ''}: ${ctx.parsed.y !== null ? ctx.parsed.y + ' kWh' : ''}` }
        };
      }
  ...
  const getWeatherIcon = useCallback((c) => {
    ...
    return (
      <span className={className} aria-label={c} role="img">
        {icon}
      </span>
    );
  }, []);
  ...
  const shareChart = useCallback(
    (platform) => {
      ...
      // Obtendo a instância do gráfico corretamente
      const chartInstance = chartRef.current.chart || chartRef.current;

      if (!chartInstance || !chartInstance.toBase64Image) {
        alert('Erro ao gerar a imagem do gráfico.');
        return;
      }

      const imageBase64 = chartInstance.toBase64Image();
      const title = 'Gráfico de Produção Solar',
        summary = 'Confira o gráfico de produção de energia solar gerada pelo nosso sistema. É um passo importante para um futuro mais sustentável!';
  ...
  const getChartData = useCallback(
    (type) => {
      ...
      const base = productionData.datasets[0],
        common = { label: base.label, data: base.data, borderColor: '#1E90FF', borderWidth: 1 };
  ...
  const generateDailyReport = useCallback(() => {
    ...
    const data = productionData.datasets[0].data;
        ...
        <section className="production-section">
          ...
          <section className="daily-report-section" style={{ marginTop: '20px' }}>
            <button
              ...
              style={{ padding: '10px 20px', cursor: 'pointer', borderRadius: '5px', border: 'none', backgroundColor: '#252525', color: '#fff', fontWeight: 'bold' }}
            >
              Relatório Diário
            </button>
          </section>
            ...
            {forecast.map((item, i) => (
              <div key={i} className="wf-card">
                ...
                <p className="wf-range">
                  Min: {item.low}°C | Max: {item.high}°C
                </p>
      ...
      {showReport && (
        <div className="report-modal" onClick={() => setShowReport(false)}>
          <div className="report-content" onClick={(e) => e.stopPropagation()}>
            <button className="closeBtn" onClick={() => setShowReport(false)} title="Fechar">
              X
            </button>
            <pre
              style={{
      ...
      {expandedChartType && (
        <div className="expanded-chart-overlay" onClick={() => setExpandedChartType(null)}>
          <div className="expanded-chart-container" onClick={(e) => e.stopPropagation()}>
            <button className="close-button" onClick={() => setExpandedChartType(null)} title="Fechar">
              X
            </button>
            <div style={{ height: '400px', width: '100%', backgroundColor: '#252525', borderRadius: '8px', padding: '15px' }}>
            ...
            <div className="share-buttons-container" style={{ marginTop: '10px' }}>
              <button
                className="share-button"
                title="Compartilhar por Email"
                onClick={() => shareChart('email')}
              >
                <img src={logoGmail} alt="Email" />
              </button>
              <button
                className="share-button"
                title="Compartilhar por WhatsApp"
                onClick={() => shareChart('whatsapp')}
              >
                <img src={logoWhatsapp} alt="WhatsApp" />
              </button>
              <button
                className="share-button"
                title="Compartilhar por Instagram"
                onClick={() => shareChart('instagram')}
              >
                <img src={logoInstagram} alt="Instagram" />
              </button>
              <button
                className="share-button"
                title="Compartilhar por LinkedIn"
                onClick={() => shareChart('linkedin')}
              >
