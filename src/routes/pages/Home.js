...
const Home = ({ productionData, onUpdateProductionData }) => {
  ...
  // Referências separadas para cada tipo de gráfico
    ...
    plugins: {
      ...
      ...(type === 'pie' && {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (context) => `${context.label}: ${context.raw} kWh (${Math.round((context.raw / context.dataset.data.reduce((a, b) => a + b, 0)) * 100)}%)`
          }
        }
      }),
      ...(type !== 'pie' && {
  ...
  const generateRandomData = () => {
    const dataLength = 7;
    const maxProduction = 40;
    const minProduction = 0;
  ...
  const handleSaveChart = () => {
    ...
    if (expandedChart) {
      // Para gráficos expandidos, usamos a referência do gráfico expandido
      chartInstance = expandedChartRef.current;
    } else {
      // Para gráficos não expandidos, usamos a referência baseada no tipo
      switch(chartType) {
        case 'line': chartInstance = lineChartRef.current; break;
        case 'bar': chartInstance = barChartRef.current; break;
        case 'pie': chartInstance = pieChartRef.current; break;
        default: chartInstance = lineChartRef.current;
      }
    }

    if (chartInstance) {
      const link = document.createElement('a');
      link.download = `grafico_producao_solar_${expandedChart || chartType}.png`;
  ...
  const shareChart = (platform) => {
    let chartInstance = null;
    let chartToShare = expandedChart || chartType;
    ...
    switch(platform) {
      case 'email':
        window.open(`mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(summary)}&attachment=${imageUrl}`);
        break;
      case 'whatsapp':
        window.open(`https://wa.me/?text=${encodeURIComponent(`${summary} ${imageUrl}`)}`);
        break;
      case 'instagram':
        // Instagram não suporta compartilhamento direto via URL, então sugerimos salvar a imagem
        handleSaveChart();
        alert('Salve a imagem e compartilhe manualmente no Instagram.');
        break;
      case 'linkedin':
        window.open(`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(imageUrl)}&title=${encodeURIComponent(title)}&summary=${encodeURIComponent(summary)}`);
        break;
      default:
        break;
    }
            ...
            <div className="chart-container" onClick={() => setState(prev => ({ ...prev, expandedChart: chartType }))}>
              {ChartComponent &&
                <ChartComponent
                  ...
                  ref={
                    chartType === 'line' ? lineChartRef :
                    chartType === 'bar' ? barChartRef :
                    pieChartRef
                  }
                />
              }
            ...
            <div style={{ backgroundColor: '#252525', borderRadius: '8px', padding: '15px', height: 'calc(100% - 170px)', overflowY: 'auto' }}>
              <div className="chart-container">
                {ExpandedChartComponent &&
                  <ExpandedChartComponent
                    data={getChartData(expandedChart)}
                    options={getChartOptions(expandedChart)}
